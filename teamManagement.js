...
function onOpen() {
	ui.createMenu("🚀 Aircall 🚀")
		...
		.addItem("Sync Team List", "syncTeams")
		.addSeparator()
		.addItem("Create Team Management", "createTeamManagement")
		.addItem("Sync Team Management", "syncTeamManagement")
		.addToUi();
}
...
async function addRecords(object, data) {
	if (
		...
		if (SpreadsheetApp.getActive().getSheetByName(object) == null)
			SpreadsheetApp.getActive().insertSheet(object);
		...
		for (r = 0; r < objectRecords.length; r++) {
			...
			recordFields.forEach(function (recordField) {
				...
				if (Array.isArray(recordField) == true)
					finalRow.push(JSON.stringify(recordField));
		...
		SpreadsheetApp.getActive()
			.getSheetByName(object)
			.getRange(2, 1, objectRecords.length, cols.length)
...
async function createTeamManagement() {
	const users = SpreadsheetApp.getActive()
		...
		.getSheetValues(
			2,
			1,
			SpreadsheetApp.getActive().getSheetByName("users").getLastRow() - 1,
			4
		);
	const teams = SpreadsheetApp.getActive()
		...
		.getSheetValues(
			2,
			1,
			SpreadsheetApp.getActive().getSheetByName("teams").getLastRow() - 1,
			2
		);
	if (SpreadsheetApp.getActive().getSheetByName("team plan") == null)
		SpreadsheetApp.getActive().insertSheet("team plan");
	...
	const logInLogOutRule = SpreadsheetApp.newDataValidation()
		.requireValueInList(["Logged In", "Logged Out"], true)
		.build();
	teamPlanTab
		.getRange(2, 2, users.length, teams.length)
		.setDataValidation(logInLogOutRule);
}
...
async function syncTeamManagement() {
	...
	const teamsPlan = SpreadsheetApp.getActive()
		...
		.getSheetValues(
			...
			2,
			1,
			SpreadsheetApp.getActive().getSheetByName("team plan").getLastColumn() - 1
		);
	const usersPlan = SpreadsheetApp.getActive()
		...
		.getSheetValues(
			2,
			1,
			SpreadsheetApp.getActive().getSheetByName("team plan").getLastRow() - 1,
			1
		);
	...
	for (let tp = 0; tp < teamsPlan[0].length; tp++) {
		...
		const parenthesisTeamData = [
			...teamsPlan[0][tp].matchAll(reParenthesis),
		].flat();
		...
		if (teamUsers.length > 0) {
			for (let tu = 0; tu < teamUsers.length; tu++) {
				...
				for (let up = 0; up < usersPlan.length; up++) {
					const parenthesisUserData = [
						...usersPlan[up][0].matchAll(reParenthesis),
					].flat();
...
async function changeTeam(teamId, userId, method) {
	let req = await UrlFetchApp.fetch(
		baseUrl + "teams/" + teamId + "/users/" + userId,
		{
...
function cellChange() {
	...
	if (cellValue === "Logged Out" || cellValue === "Logged In") {
		...
		const aircallUserId = [...cellUser.matchAll(reParenthesis)]
			.flat()
			.slice(-1)[0];
		const aircallTeamId = [...cellTeam.matchAll(reParenthesis)]
			.flat()
			.slice(-1)[0];
		if (aircallUserId == "")
			ui.alert(
				"User Name and ID is not correctly formatted. Please create and sync the team plan again"
			);
		// add user to team
