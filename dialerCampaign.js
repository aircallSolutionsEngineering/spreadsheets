...
async function createDialerCampaign() {
  ...
  for (p = 0; p < selectedPhoneNumbers.length; p++) {
    if (selectedPhoneNumbers[p][0] !== "") {
      ...
      if (cleanPhoneNumber.includes("+") != true)
    ...
    if (activeUserAircallId === "")
      ui.alert(
        "👎👎👎Error👎👎👎\r\nCant create the Dialer Campaign\r\n\r\n" +
          "No Aircall user found with Google user: " +
          activeUserEmail
      );
    else {
      // Logger.log(activeUserAircallId);
      // check if existing dialer campaign for user
      res = await UrlFetchApp.fetch(
        baseUrl + "users/" + activeUserAircallId + "/dialer_campaign",
        {
      ...
      if (res.getResponseCode() !== 404) {
        ...
        let res = await UrlFetchApp.fetch(
          baseUrl + "users/" + activeUserAircallId + "/dialer_campaign",
          {
...
async function delegateDialerCampaign(agent) {
  ...
  try {
    ...
    let res = await UrlFetchApp.fetch(
      baseUrl +
        "users/" +
        userDict.find((u) => u.name === agent).aircall_id +
        "/dialer_campaign",
      {
        method: "GET", // *GET, POST, PUT, DELETE, etc.
        headers: {
